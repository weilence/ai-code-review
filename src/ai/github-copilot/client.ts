...
export class CopilotAuthClient {
  ...
  async requestDeviceCode(): Promise<DeviceCodeResponse> {
    ...
    if (!response.ok) {
      throw new CopilotAuthError(`Failed to request device code: ${response.status}`);
    }

    const result = DeviceCodeResponseSchema.safeParse(await response.json());

    if (!result.success) {
      throw new CopilotAuthError('Invalid response from GitHub');
    }
  ...
  async pollForAccessToken(deviceCode: string): Promise<OAuthTokenResponse | OAuthError> {
    ...
    if (!response.ok) {
      throw new CopilotAuthError(`Failed to poll for access token: ${response.status}`);
    }
    ...
    throw new CopilotAuthError('Unexpected response from GitHub');
  }
  ...
  async getCopilotToken(accessToken: string): Promise<CopilotTokenResponse> {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new CopilotAuthError('Invalid or expired access token', 'unauthorized');
      }
      if (response.status === 403) {
        throw new CopilotAuthError('Access denied - user may not have Copilot access', 'forbidden');
      }
      throw new CopilotAuthError(`Failed to get Copilot token: ${response.status}`);
    }

    const result = CopilotTokenResponseSchema.safeParse(await response.json());

    if (!result.success) {
      throw new CopilotAuthError('Invalid response from Copilot API');
    }
